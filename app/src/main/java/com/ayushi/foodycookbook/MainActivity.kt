...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var idMeal: String = intent.getStringExtra("idMeal").toString()

        val queue = Volley.newRequestQueue(this)
        val url = "https://www.themealdb.com/api/json/v1/1/lookup.php?i=$idMeal"
        val json_Object_Request =
            object : JsonObjectRequest(Request.Method.GET, url, null, Response.Listener {
                val meals = it.getJSONArray("meals")
                strMeal = meals.getJSONObject(0).getString("strMeal")
                txtStrMeal.text = strMeal
                supportActionBar?.title = strMeal

                var imageUrl = meals.getJSONObject(0).getString("strMealThumb")
                vidUrl = meals.getJSONObject(0).getString("strYoutube")
